...
let duration = 5000;
let str = ['HAPPY', 'VALENTINE', 'DAY' , 'ADHITYA'];
...
function firework(t, i, pts) {
    ...
    // 💖 POSISI HURUF DIBUAT CENTER
    let spacing = Math.min(w / (chars.length + 1), 160);
    let totalWidth = spacing * (chars.length - 1);
    let startX = (w - totalWidth) / 2;
    let dx = startX + i * spacing;

    // Gerakan goyang kecil biar hidup
